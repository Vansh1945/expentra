...
const AddGroupExpense = () => {
    ...
    return (
        <div className="min-h-screen pb-12 text-textColor">
            {/* Header */}
            <div className="bg-transparent px-4 py-4 mb-6">
                <div className="max-w-4xl mx-auto flex items-center gap-3">
                    <button
                        onClick={() => navigate('/groups/expenses')}
                        className="p-2 rounded-lg bg-card border border-slate-200 hover:bg-primary hover:text-white transition-all duration-200 shadow-sm group"
                    >
                        <MdArrowBack className="w-4 h-4 transition-transform group-hover:-translate-x-0.5" />
                    </button>
                    <div>
                        <h1 className="h2-premium">
                            {isEditMode ? 'Edit Expense' : 'Record Group Expense'}
                        </h1>
                        <p className="small-premium mt-0.5">{groupData?.name || 'Group Activity'}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-4xl mx-auto px-4">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        ...
                        <div className="bg-card rounded-xl border border-slate-100 p-4 shadow-sm">
                            <h2 className="text-xs font-bold uppercase tracking-widest text-primary flex items-center gap-1.5 mb-3">
                                <MdReceipt className="text-base" /> Expense Details
                            </h2>
                        ...
                        {/* Metadata Details */}
                        <div className="bg-card rounded-xl border border-slate-100 p-4 shadow-sm space-y-3">
                            <h4 className="text-xs font-bold uppercase tracking-widest text-primary flex items-center gap-1.5">
                                <MdCategory className="text-base" /> Extra Details
                            </h4>
                                ...
                                <div className="relative group">
                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none z-10">
                                        <CategoryIcon
                                            ...
                                            className="text-primary w-4 h-4 transition-transform group-hover:scale-105"
                                        />
                                    </div>
                                    <select
                                        value={category}
                                        onChange={e => setCategory(e.target.value)}
                                        className="select-premium pl-9 appearance-none cursor-pointer"
                                    >
                                        {categories.length > 0 ? (
                                            <>
                                                {categories.map(c => (
                                    ...
                                    <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                        <MdOutlineCategory className="text-textColor/30 w-4 h-4" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                ...
                                <div className="space-y-1">
                                    <label className="label-premium">Remarks (Optional)</label>
                                    <input
                                        type="text"
                                        value={note}
                                        onChange={e => setNote(e.target.value)}
                                        placeholder="Write something..."
                                        className="input-premium placeholder:opacity-50"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-card rounded-xl border border-slate-100 p-4 shadow-sm">
                            <div className="flex justify-between items-center mb-3">
                                <h2 className="text-xs font-bold uppercase tracking-widest text-primary flex items-center gap-1.5">
                                    <div className="w-5 h-5 rounded bg-primary text-white text-[9px] flex items-center justify-center font-black">1</div>
                                    Who Paid?
                                </h2>
                                <p className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded-full ${isPaidCorrect ? 'bg-success/10 text-success' : 'bg-danger/10 text-danger'}`}>
                            ...
                            <div className="space-y-1.5">
                                {groupData?.members.map(m => {
                                    ...
                                    return (
                                        <div key={m._id} className="group flex items-center justify-between px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-lg hover:border-primary/20 transition-all">
                                            <div className="flex items-center gap-2">
                                                <div className="w-6.5 h-6.5 rounded bg-primary/5 flex items-center justify-center text-primary font-bold text-xs uppercase">
                                                    {m.name.charAt(0)}
                                                </div>
                                                <span className="font-semibold text-xs text-textColor">{m.name}</span>
                                            </div>
                                            <div className="flex items-center gap-1">
                                                <span className="text-primary text-[10px] font-bold opacity-40">₹</span>
                                                <input
                                                    type="number"
                                                    value={payer ? payer.amount : ''}
                                                    onChange={e => handlePayerChange(m._id, m.name, m.user, e.target.value)}
                                                    placeholder="0.00"
                                                    className="w-20 px-2 py-1 rounded bg-card border border-slate-200 focus:border-primary/35 focus:outline-none text-right text-xs font-bold transition-all h-8"
                                                />
                                            </div>
                                        </div>
                                    );
                        ...
                        <div className="bg-card rounded-xl border border-slate-100 p-4 shadow-sm">
                            <h2 className="text-xs font-bold uppercase tracking-widest text-primary mb-3 flex items-center gap-1.5">
                                <div className="w-5 h-5 rounded bg-primary text-white text-[9px] flex items-center justify-center font-black">2</div>
                                Split Method
                            </h2>

                            <div className="flex gap-2 mb-3 p-1 bg-slate-50 rounded-xl border border-slate-100">
                                {['equal', 'exact', 'percentage'].map(type => (
                                    <button
                                        key={type}
                                        onClick={() => setSplitType(type)}
                                        className={`flex-1 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all ${splitType === type
                            ...
                            <div className="space-y-1.5">
                                {splitDetails.map(m => (
                                    <div
                                        key={m.mid}
                                        className={`flex items-center justify-between px-3 py-1.5 rounded-lg border transition-all ${m.involved
                                            ? 'bg-slate-50 border-primary/10 shadow-sm'
                                            : 'bg-card border-slate-100 opacity-40'
                                            }`}
                                    >
                                        <div className="flex items-center gap-2.5">
                                            ...
                                            />
                                            <span className={`text-xs font-bold ${m.involved ? 'text-textColor' : 'text-textMuted'}`}>
                                                {m.name}
                                            </span>
                                        </div>
                                        ...
                                        {m.involved && (
                                            <div className="flex items-center gap-1">
                                                {splitType === 'equal' ? (
                                                    <div className="px-2 py-0.5 rounded bg-primary/5 text-primary text-xs font-bold">
                                                        ₹{(Number(amount) / involvedCount || 0).toFixed(2)}
                                                    </div>
                                                ) : (
                                                    <>
                                                        <span className="text-primary text-[9px] font-black opacity-30">
                                                        ...
                                                        <input
                                                            ...
                                                            placeholder="0"
                                                            className="w-16 px-2 py-1 rounded bg-card border border-slate-205 focus:border-primary/30 focus:outline-none text-right text-xs font-bold transition-all h-8"
                                                        />
                                                    </>
                                                )}
                                            </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {/* Summary Card */}
                        <div className="bg-gradient-to-br from-indigo-600 to-indigo-700 rounded-xl p-4 shadow-sm text-white group">
                            <h3 className="font-bold text-xs uppercase tracking-widest mb-3 flex items-center gap-1.5 opacity-80">
                                <MdCalculate className="text-base" /> Overview
                            </h3>
                            ...
                            <div className="space-y-3 text-xs mb-4">
                                <div className="flex justify-between items-baseline border-b border-white/10 pb-1.5">
                                    ...
                                    <span className="font-black text-lg">₹{amount || '0.00'}</span>
                                </div>
                                <div className="flex justify-between items-baseline border-b border-white/10 pb-1.5">
                                    <span className="opacity-70 font-medium">Recorded Paid</span>
                                    <span className={`font-black ${isPaidCorrect ? 'text-white' : 'text-danger/70 animate-pulse'}`}>
                                        ₹{totalPaid.toFixed(2)}
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="opacity-70 font-medium">Shared by</span>
                                    <span className="font-bold">{involvedCount} People</span>
                                </div>
                            </div>

                            <button
                                onClick={handleSubmit}
                                className="w-full py-2 bg-white text-primary hover:bg-slate-50 rounded-lg font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all shadow-md active:translate-y-0"
                            >
                                {isEditMode ? <MdEdit className="text-sm" /> : <MdCheckCircle className="text-sm" />}
